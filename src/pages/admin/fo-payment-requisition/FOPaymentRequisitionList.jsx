...
import ViewPaymentRequisition from "./ViewFOPaymentRequisition";
import CheckAccessEdit from "../../../components/common/CheckAccessEdit";
import toast from "react-hot-toast";
import { FaList } from "react-icons/fa6";

export default function FOPaymentRequisitionList() {
  ...
  const { data: foPaymentList = [], isLoading } = useQuery({
    queryKey: ["payment-requisition-list", refetchList, modal, fincYear],
    queryFn: async () => {
      return await getFOPaymentRequisitionList(token, fincYear);
    },
  });
  ...
  const openModal = (id) => {
    const allPayments = foPaymentList?.response;
    const singlePayment = allPayments.find((payment) => payment.id === id);
    setSinglePaymentData(singlePayment);
    setModal(true);
  };

  const filteredPayments = foPaymentList?.response?.filter((payment) => {
  ...
  // Export to Excel
  //   const exportToExcel = () => {
  //     const ws = XLSX.utils.json_to_sheet(foPaymentList?.response || []);
  ...
  //         ],
  //       ],
  //       body: foPaymentList?.response.map((payment) => [
  ...
  return (
    <SidebarProvider>
      <AppSidebar />
      <SidebarInset>
        <AdminHead breadcrumb_name="FOPayment Requisition" />
                    ...
                    <Column
                      ...
                      body={(rowData) => (
                        <>
                          ...
                          <CheckAccessEdit edit_access="Fo Payment Requisition">
                            <TooltipProvider>
                              <Tooltip>
                                <TooltipTrigger>
                                  <button
