...
import { editRole } from "../../../services/api";  // Assuming you have a similar editRole API
...
export default function EditRole({ role }) {
    ...
    const editRoleMutation = useMutation({
        mutationFn: async (data) => {
            ...
            return await editRole(token, role.id, data.role_name, access_add, access_edit, data.status === "active" ? "1" : "0");  // Convert status to "1" (Active) or "0" (Inactive)
        },
        onSuccess: (value) => {
                            ...
                            {accessOptions.map((option) => (
                                <div key={option} className="flex items-center gap-1 px-5 py-1  rounded-3xl bg-whitesmoke shadow-lg">
                                    <input
                                        ...
                                        defaultChecked={role.access_type_add?.includes(option)}
                                    />
                            ...
                            {accessOptions.map((option) => (
                                <div key={option} className="flex items-center gap-1 px-5 py-1  rounded-3xl bg-whitesmoke shadow-lg">
                                    <input
                                        ...
                                        defaultChecked={role.access_type_edit?.includes(option)}
                                    />
